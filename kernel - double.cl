...
inline double lookup(double t,_data_type_ *sinc_table)//normallized, real t
{
...
int beg_index,int out_samples,int chn_num,int ori_sr,int des_sr,int w_hn, __global _data_type_ *sinc_table_)
{
	...
	//interpolation
	{
		...
		for(int i=0;i<w_hn;i++){//right wing
			...
			nIVL+=(double)((double)vL*lookup(ti-(double)index_f,sinc_table));//sinc_GPU(fabs(ti-(double)index_f),1,ratio)*windows_GPU(fabs(ti-(double)index_f),(double)w_hn));
		}
		for(int i=0;i<w_hn;i++){//right wing
			//window size 32
			int nId=a_nL-i;
