...
typedef struct {
  ...
  FFTComplex * tmp_buf; // for MSVS 2010 debugger: dft_filter_t was made identical in rate_dbl and rate_flt
} dft_filter_t;
...
{                    /* TODO this could be cleaner, esp. for k != 0 */
  ...
  //if (k && n==10) n=12; //flatter passband; testin'
    ...
    int num_phases, int interp_order, int multiplier)
{
  ...
  if (interp_order==2) {  //d150_2, u150_2
    ...
    sox_sample_t t;
    for(i=0; i < num_coefs*num_phases/2; i++) {
      t = c[6*i + 1]; c[6*i + 1] = c[6*i + 3]; c[6*i + 3] = c[6*i + 4]; c[6*i + 4] = c[6*i + 2]; c[6*i + 2] = t;
    }
  }
  /*if (interp_order==1) {  //d120_1, u120_1  --  now we don't use it
    sox_sample_t * const c = result;
    sox_sample_t t;
    for(i=0; i < num_coefs*num_phases/2; i++) {
...
#define CAT2(a,b) a##b
#ifdef SSE3_
#define CAT_POSTFIX(a) CAT2(a, _SSE3)
#else
#define CAT_POSTFIX(a) CAT2(a, _generic)
#endif

#define lsx_safe_rdft_x CAT_POSTFIX(lsx_safe_rdft)
#define x_lsx_safe_rdft_x CAT_POSTFIX(x_lsx_safe_rdft)
...
static void half_sample(stage_t * p, fifo_t * output_fifo)
{
...
#ifdef SSE3_
  const double * const coeff = f->coefs;
  __m128d xmm0, xmm1;
#endif
  ...
  while (num_in >= f->dft_length) {
    ...
    lsx_safe_rdft_x(f->dft_length, 1, output);
#ifdef SSE3_
    xmm0 = _mm_load_pd(output);
    xmm1 = _mm_load_pd(coeff);
    _mm_store_pd(output, _mm_mul_pd(xmm1, xmm0));
    for (i = 2; i < f->dft_length; i += 2)
    {
      xmm0 = _mm_load_pd(output+i);
      xmm1 = _mm_load_pd(coeff+i);
      _mm_store_pd(output+i, ZMUL(xmm0, xmm1));
    }
#else
    output[0] *= f->coefs[0];
    output[1] *= f->coefs[1];
    ...
    int multiplier, double phase, sox_bool allow_aliasing)
{
  ...
  lsx_safe_rdft_x(dft_length, 1, f->coefs);
}

#ifdef SSE3_
#include "rate_filters_sse3.h"
#else
#include "rate_filters_dbl.h"
#endif
    ...
    sox_bool allow_aliasing, sox_bool old_behaviour)
{
  ...
  {
    double att = allow_aliasing? (34./33) * 170. : 170.; /* negate att degrade */
    double bw = bandwidth? 1 - (1 - bandwidth / 100) / LSX_TO_3dB : .931;
    double min = 1 - (allow_aliasing? LSX_MAX_TBW0A : LSX_MAX_TBW0) / 100;
    init_dft_filter(shared, p->upsample, 0, NULL, bw, 1., (double)max(tuple, two_or_three), att, mult, phase, allow_aliasing);
    if (p->upsample) {
...
#define RR_init_x CAT_POSTFIX(RR_init)
#define RR_flow_x CAT_POSTFIX(RR_flow)
#define RR_drain_x CAT_POSTFIX(RR_drain)
#define RR_close_x CAT_POSTFIX(RR_close)
#define RR_reset_x CAT_POSTFIX(RR_reset)
