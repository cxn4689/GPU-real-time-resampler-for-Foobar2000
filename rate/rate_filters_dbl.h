...
static const double half_fir_coefs_25[] = {
...
static void half_sample_25(stage_t * p, fifo_t * output_fifo)
{
  ...
  {
    int i;
    double const * input = ((double *)fifo_read_ptr(&p->fifo) + p->pre);
...
static void d150_0(stage_t * p, fifo_t * output_fifo)
{
  ...
  if (num_in > 0)
  {
    ...
    for (i = 0; p->at.parts.integer < num_in * p->divisor; ++i, p->at.parts.integer += p->step.parts.integer)
    {
      ...
      double const * at = input + divided.quot;
      double const * coef = &p->shared->poly_fir_coefs[d150_l*divided.rem];
...
static void d150_2(stage_t * p, fifo_t * output_fifo)
{
  ...
  if (num_in > 0)
  {
    ...
    {
      double const * at = input + p->at.parts.integer;
      uint32_t fraction = p->at.parts.fraction;
      int phase = fraction >> (32 - d150_2_b);
      double const * coef = &p->shared->poly_fir_coefs[d150_l*3*phase];

      double x = (double) (fraction << d150_2_b) * (1 / MULT32);
...
static void u150_0(stage_t * p, fifo_t * output_fifo)
{
  ...
  if (num_in > 0)
  {
    ...
    for (i = 0; p->at.parts.integer < num_in * p->divisor; ++i, p->at.parts.integer += p->step.parts.integer)
    {
      div_t divided = div1(p->at.parts.integer, p->divisor);
      double const * at = input + divided.quot;
      double const * coef = &p->shared->poly_fir_coefs[u150_l*divided.rem];
      double sum = 0;
      int j = 0;
...
static void u150_2(stage_t * p, fifo_t * output_fifo)
{
  ...
  if (num_in > 0)
  {
    ...
    for (i = 0; p->at.parts.integer < num_in; ++i, p->at.all += p->step.all)
    {
      ...
      double const * coef = &p->shared->poly_fir_coefs[u150_l*3*phase];

      double x = (double) (fraction << u150_2_b) * (1 / MULT32);
